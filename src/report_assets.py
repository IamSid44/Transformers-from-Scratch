...
def main() -> None:
    ...
    chunks_per_line = 9322 / 500

    parts = ["# Report materials\n",
             "Generated by `src/report_assets.py`. Numbers only -- the write-up is by hand.\n",
             "\n## How to read the four tables\n",
             "| Table | Question it answers |\n| --- | --- |\n"
             "| Quality per chunk | How good is the model at the unit it actually decodes? "
             "**This is the headline table.** |\n"
             "| Quality per line | What happens when ~19 chunk outputs are glued back into a "
             "document? |\n"
             "| Cost | What does each configuration cost in time and memory? |\n"
             "| Convergence | Did each run finish, and did it learn to read its source? |\n",
             f"\nThe per-line numbers look far worse, but they are the **same predictions** "
             f"scored on a harder unit, not a different or worse model. A test line is "
             f"{chunks_per_line:.1f} chunks, so (a) a line counts as correct only if every one "
             f"of its ~19 chunks is simultaneously perfect -- at C1's 95.34% per chunk that is "
             f"0.9534^{chunks_per_line:.1f} = 41%, and C1 scores 51.6%, better than "
             f"independence predicts because errors cluster in hard lines; and (b) Levenshtein "
             f"is a *count* of edits, so it accumulates: 0.067 x {chunks_per_line:.1f} = 1.26 "
             f"against 1.26 observed, and for C5 0.727 x {chunks_per_line:.1f} = 13.56 against "
             f"13.55 observed. The per-line table is derivable from the per-chunk one.\n",
             "\nUse the per-chunk table for the ablation. The per-line table is worth "
             "reporting for exactly two observations: the C5 length-faithfulness result below, "
             "and the fragility of positional metrics (character accuracy collapses under a "
             "single length slip, while Levenshtein and BLEU barely move).\n",
