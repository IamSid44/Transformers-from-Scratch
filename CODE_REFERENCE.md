...
**`TrainConfig`** — shared by *all five* configurations: 60 epochs, batch size 1024, Adam at
`lr=1e-3` with `warmup_epochs=1` (converted to a step count in `train_one`, since
`steps_per_epoch` depends on `batch_size` — fixing a raw step count would silently mean a
different warmup fraction at a different batch size; 39 steps at the current batch size),
